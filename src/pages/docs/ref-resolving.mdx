...
<Note
  title='ATTENTION'
  body='Ref resolving is only supported out of the box within the React renderers.'
/>
...
To do, use the `refParserOptions` and `ajv` props of the `JsonForms` component and pass in the ref resolver together with the AJV instance.
