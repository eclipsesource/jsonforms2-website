...
Withing the same form, inputs can be interlinked and interact with each other by using `$ref` and setting the `scope`
...
There might be use cases where you have forms that do not have anything in common, so your forms are independent, although they can share the same context.
