---
name: Create a JSON Forms App (Redux)
route: /docs/tutorial-redux
parent: Tutorials
---
...
1. We'll use [create-react-app](https://github.com/facebookincubator/create-react-app) to scaffold a basic React application which we'll use as a starting point.
   ...
   ```bash
   npx create-react-app my-jsonforms-app --typescript
   ```
...
3. Switch to the `src` directory and open `index.js` (`index.tsx` when using typescript) with an editor of your choice.
...
4. Still in `index.js` (`index.tsx` when using typescript), create a new store with the `createStore` function provided by Redux.
...
6. Open the `App.js` (`App.tsx` when using typescript) file and import the `JsonForms` component from `@jsonforms/react`.
