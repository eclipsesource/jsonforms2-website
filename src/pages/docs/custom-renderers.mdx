...
It's contained in `src/Rating.js` and is a rating control, i.e. it allows to set a value between 0 and 5
...
via another helper function provided by JSON Forms called `mapDispatchToControlProps`.
All the handler actually does is to emit a change with the new value.

The complete code of `src/RatingControl.js` looks as follows:
