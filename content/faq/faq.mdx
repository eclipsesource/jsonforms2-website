...
## How can I minimize re-rendering in React?
...
There are three exceptions: JSON Forms can handle "onChange" and "middleware" changes, for example to support anonymous functions. Also new "i18n" objects will be ignored as long as its properties are stable.
...
Only provide a new object to JSON Forms if necessary, for example if an object needs to be modified.

React also provides other tools to stabilize data, such as the `useMemo`, `useCallback`, or `useRef` hooks, to improve the performance.
