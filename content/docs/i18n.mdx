...
Please note that the renderer sets themselves are not yet translated.
...
It can be set using the `UI Schema`, the `JSON Schema` or by default is generated based on the property path.
...
The key can also be set with the JSON Schema `i18n` option:
```json
{
  "name": { 
    "type": "string", 
...
### `defaultMessage`
The default message is provided by JSONForms and can act as a fallback.

:::note

If the defaultMessage is `undefined` it should not be modified and still return `undefined` (and not an empty string or similar).
