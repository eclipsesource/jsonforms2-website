...
And to implement the `handleChange` function, we need access to the `x-dependencies` field in the schema.

#### Accessing Schema Data

The `x-endpoint` and `x-dependencies` fields can be obtained from the schema object provided to the custom renderer via JSON Forms.
...
...
  ...
  const dependent = schema['x-dependencies'];
...
}
```
...
const CountryControl = (
  ...
  const endpoint = schema['x-endpoint'];
  const dependent: string[] = schema['x-dependencies'] ? schema['x-dependencies'] : [];
...
It also accesses the API via the context and includes `x-endpoint` and `x-dependencies` fields defined in its schema.
