...
about [Custom Renderers](/docs/tutorial/custom-renderers).
...
The `showSortButtons` option is used to toggle additional buttons that allow changing the
...
When using the `readonly` option, you can disable the input to this field. See [example](/examples/readonly#ui-schema).
