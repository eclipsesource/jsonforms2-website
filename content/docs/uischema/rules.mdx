...
In this example, the rule evaluates to true if the `scope` property `name` is either "foo" or "bar".
...
To match a scope variable to a specific value use "const":
...
You can negate a rule by using "not":
