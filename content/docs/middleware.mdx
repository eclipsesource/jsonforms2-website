...
In this tutorial, we'll introduce the JSON Forms reducer pattern and its key actions.
...
The middleware's arguments are the current JSON Forms state, the disptached action and the default reducer of JSON Forms.
...
The default reducer can be used in cases where the behavior of JSON Forms shouldn't be altered:
...
In the following, we will explore two examples demonstrating how this middleware can be utilized to provide custom implementations for JSON Forms actions.
...
We additionally override the `INIT` and `UPDATE_CORE` actions, in case the data prop passed to JSON Forms dosnt have the correct price set yet.
